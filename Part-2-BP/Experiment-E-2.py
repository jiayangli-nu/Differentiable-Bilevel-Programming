...
""""
This script only tests EP and CPU
"""
...
for jj, N in enumerate(city_size):
    ...
    m = len(cap)
    l = max(p_loc) + 1
    path_edge = sparse.FloatTensor(i, v, torch.Size([m, l])).to(device).to_dense()
    ...
    m = len(od_data)
    l = max(p_loc) + 1
    path_demand = sparse.FloatTensor(i, v, torch.Size([m, l])).to(device).to_dense()
    ...
    while i < T_max:
        ...
        p_var = cp.Variable(len(f_a))
        b_par = cp.Parameter(len(c))
        constraints = [p_var >= 0,
                       path_demand @ p_var == 1]
        ...
        p_h = cvxpylayer(b_h)[0]
        
        b_a = p_a - r * c
        cvxpylayer = CvxpyLayer(problem, parameters=[b_par], variables=[p_var])
        p_a = cvxpylayer(b_a)[0]
        i += 1
    Iteration_num[jj] = i
    f_a = q_a * p_a
    x_a = path_edge @ f_a
...
Table = PrettyTable()
Table.field_names = ['|N|', '10 x 10', '20 x 20']
Table.add_rows(
        [
                ["|K^*|"] + list(Path_num[1:]),
...
print('Computational performance of FP and BP by EP ')
print(Table)
