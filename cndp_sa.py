...
def cost_function(cap_add):
    ...
    net.solve_ue(Demand, Fftime, Capacity, 1000, 1e-4, warm_start=True)
    ...
    if n_ue == 0:
        ...
        with open('result_cndp/' + 'sa' + str(i) +  '_' + NETWORK + '.pkl', 'wb') as f:
...
for NETWORK in ['bar']:
    ...
    if NETWORK == 'bar':
        N_max = 20000
        bounds = [(0, 4) for _ in range(len(enh_link))]
    if NETWORK == 'cs':
        N_max = 10000
        bounds = [(0, 3) for _ in range(len(enh_link))]
